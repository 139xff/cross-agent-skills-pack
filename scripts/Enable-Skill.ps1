...
$sourcePath = [System.IO.Path]::GetFullPath((Join-Path $disabledRoot $skillName))
$destinationPath = [System.IO.Path]::GetFullPath((Join-Path $enabledRoot $skillName))

Assert-PathWithinRoot -Path $sourcePath -Root $disabledRoot
Assert-PathWithinRoot -Path $destinationPath -Root $enabledRoot
...
if ($DryRun) {
    Write-Output "DRY-RUN: would enable skill '$skillName'."
...
Set-SkillManifestMode -Manifest $manifest -SkillName $skillName -Mode 'auto'

if ($sourceExists) {
    if (-not (Test-Path -LiteralPath $enabledRoot -PathType Container)) {
        New-Item -ItemType Directory -Path $enabledRoot -Force | Out-Null
    }
...
Write-Output "Enabled skill '$skillName'."
& $syncScript @syncParameters
